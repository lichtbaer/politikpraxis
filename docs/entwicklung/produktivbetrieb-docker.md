...
- **Per-Worker-Limits:** Das Backend startet in Produktion mit `--workers 2` ([`docker-compose.prod.yml`](https://github.com/lichtbaer/politikpraxis/blob/main/docker-compose.prod.yml), Backend-`command`). In-Memory-Buckets (slowapi/Auth, Kontakt-Limiter) gelten weiterhin **pro Worker**; die effektiven Limits können sich dadurch annähernd verdoppeln.
- **Admin-Rate-Limit (worker-/instanzübergreifend, #231):** Das Admin-Rate-Limit (30 Anfragen/Minute pro IP, `backend/app/services/admin_rate_limit.py`) liegt in Postgres statt im Prozess-Speicher — ein Fixed-Window-Zähler pro IP, atomar per `INSERT ... ON CONFLICT` fortgeschrieben. Das Limit greift dadurch über alle Worker-Prozesse und Instanzen hinter dem Load-Balancer hinweg und übersteht Neustarts. Trade-off gegenüber dem vorherigen Sliding-Window: An Fenstergrenzen (60 s) sind kurzzeitig knapp doppelt so viele Anfragen möglich — für ein Brute-Force-Limit auf Admin-Endpunkten bewusst in Kauf genommen. Die übrigen In-Memory-Limiter (Auth/slowapi, Kontaktformular) sind bewusst nicht mit umgestellt; ein geteilter Storage (Redis) dafür bleibt ein mögliches Folge-Issue.

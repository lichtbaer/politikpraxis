...
class UserTestFeedback(Base):
    ...
    game_stat_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("game_stats.id", ondelete="SET NULL"), nullable=True
    )
    kontext: Mapped[str] = mapped_column(Text(), nullable=False)  # "header" | "spielende"
    ...
    fehler_gemeldet: Mapped[bool] = mapped_column(Boolean(), nullable=False, server_default="false")

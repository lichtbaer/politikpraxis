...
def upgrade() -> None:
    ...
    gesetze_data = [
        ...
        ("nahverkehr_ausbau", ["bund", "kommunen"], 52, -0.4, -0.3, -0.2, 5, 4, False, -40, -20, -30, "digital_infrastruktur", 3.0, 0.5, 0.0, True, True, False, True),
        ("kitaausbau", ["bund", "kommunen"], 58, -0.5, -0.4, -0.3, 6, 5, False, -50, -30, -40, "gesundheit_pflege", 4.0, 1.5, 0.0, True, True, False, True),
        ("stadtentwicklung", ["bund", "kommunen"], 54, -0.3, -0.2, -0.2, 4, 4, False, -35, -25, -30, "wirtschaft_finanzen", 2.0, 0.3, 0.0, True, True, False, True),
        ("gemeindeordnung_reform", ["bund", "kommunen", "land"], 50, -0.2, 0.1, 0.1, 3, 3, True, 40, 0, 0, "wirtschaft_finanzen", 0.0, 0.0, 0.0, False, True, True, True),
        ("laender_polizeigesetz", ["bund", "land"], 48, 0, -0.1, 0, 4, 4, False, 10, 60, 0, "innere_sicherheit", 0.5, 0.2, 0.0, False, False, True, True),
        ("hochschulrahmen", ["bund", "land"], 55, -0.3, -0.2, -0.2, 5, 5, False, -30, -20, -25, "bildung_forschung", 1.0, 0.5, 0.0, True, False, True, True),
        ("laenderfinanzausgleich_reform", ["bund", "land"], 46, -0.1, 0.2, 0.1, 0, 1, True, 20, 0, 0, "wirtschaft_finanzen", 0.0, 0.0, 0.0, False, False, True, True),
    ]

    for (
        gid, tags, bt, ea, eh, eg, ez, lag, foed, iw, ig, is_, pf, ke, kl, einn, inv, kpm, lpm, eim
    ) in gesetze_data:
        tags_sql = "{" + ",".join(f'"{t}"' for t in tags) + "}"
        conn.execute(
            sa.text("""
    ...
    # --- events_i18n (DE) ---
    events_i18n_de = [
        ("kommunal_haushaltskrise", "Kommunal", "Kommunale Haushaltskrise — Städte fordern Hilfe",
        ...
        ("kommunal_buergerprotest", "Kommunal", "Bürgerproteste gegen Windkraftausbau",
        ...
        ("laender_koalitionskrise", "Länder", "Länder-Koalition unter Druck",
    ...
    # --- events_i18n (EN) ---
    events_i18n_en = [
        ("kommunal_haushaltskrise", "Municipal", "Municipal budget crisis — cities demand help",
        ...
        ("kommunal_buergerprotest", "Municipal", "Citizen protests against wind power expansion",
        ...
        ("laender_koalitionskrise", "States", "State coalition under pressure",
    ...
    choices_data = [
        ...
        ("kommunal_haushaltskrise", "nothilfe", "primary", 0, -3.0, 0, 0, 0, None, "Kommunale Nothilfe (3 Mrd.)", "Sofortige Hilfen für Haushaltsnot."),
        ("kommunal_haushaltskrise", "ablehnen", "danger", 0, 0, 0, -3, 0, None, "Ablehnen — Eigenverantwortung", "Keine Reaktion. Druck steigt."),
        ("kommunal_haushaltskrise", "strukturhilfe", "safe", 15, -1.5, 0, 0, 0, None, "Strukturhilfe-Paket ausarbeiten", "15 PK — Gezielte Hilfen."),
        # kommunal_buergerprotest: dialog (10 PK, mk +3), ignorieren (mk -5), kompromiss (8 PK)
        ("kommunal_buergerprotest", "dialog", "primary", 10, 0, 0, 0, 3, None, "Dialog-Prozess starten", "10 PK — Medienklima verbessert."),
        ("kommunal_buergerprotest", "ignorieren", "danger", 0, 0, 0, 0, -5, None, "Ignorieren", "Medienklima verschlechtert."),
        ("kommunal_buergerprotest", "kompromiss", "safe", 8, 0, 0, 0, 0, None, "Abstandsregeln anpassen", "8 PK — Kompromiss."),
        # laender_koalitionskrise: einlenken (15 PK, br_bonus +10), konfrontation (br_bonus -5)
        ("laender_koalitionskrise", "einlenken", "primary", 15, 0, 0, 0, 0, 10, "Einlenken — Länder-Gipfel", "15 PK — Bundesrat-Beziehung +10."),
        ("laender_koalitionskrise", "konfrontation", "danger", 0, 0, 0, 0, 0, -5, "Konfrontation", "Bundesrat-Beziehung -5."),
    ]

    for event_id, ckey, ctype, cost, eff_hh, eff_gi, eff_zf, mk_delta, br_bonus, label, desc in choices_data:
        conn.execute(
            sa.text("""
            ...
            """),
            {
                ...
                "eff_hh": eff_hh, "eff_gi": eff_gi, "eff_zf": eff_zf,
                "mk_delta": mk_delta, "br_bonus": br_bonus,
            },
        )
    ...
    choices_i18n_en = [
        (choice_id - 8, "Emergency municipal aid (3 bn.)", "Immediate aid for budget distress."),
        ...
        (choice_id - 2, "Give in — state summit", "15 PK — Bundesrat relationship +10."),
...
def downgrade() -> None:
    ...
    # Events + Choices
    event_ids = ["kommunal_haushaltskrise", "kommunal_buergerprotest", "laender_koalitionskrise"]
    ...
    new_gesetz_ids = [
        "nahverkehr_ausbau", "kitaausbau", "stadtentwicklung", "gemeindeordnung_reform",
        "laender_polizeigesetz", "hochschulrahmen", "laenderfinanzausgleich_reform",
    ]

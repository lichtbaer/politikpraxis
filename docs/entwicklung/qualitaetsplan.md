...
|---|---|---|---|
| ✅ | **Trivy-Failure beheben**: `pip install --upgrade pip setuptools` im Backend-Image (msgpack kommt als pip-Vendor mit, setuptools 70.3 ist CVE-2025-47273). PR #423 liegt bereits vor und sollte als Erstes gemergt werden. | `backend/Dockerfile` | S | *(umgesetzt auf `claude/project-quality-plan-y9fqgf`, identisch zu #423)*
...
| ✅ | `analytics_events.save_id`: `ondelete="SET NULL"` plus Migration. Löschen eines Spielstands mit Analytics-Events liefert heute 500. | `models/analytics.py:23` | *Zusatzbefund: die Tabelle wurde von keiner Migration angelegt (036 änderte sie nur „falls vorhanden“); Migration 068 legt sie idempotent an.*
| ✅ | `event_type` als `Literal["random","char_ultimatum","bundesrat"]` validieren; der Prozess-Cache wächst sonst pro beliebigem `?type=`-Wert unbegrenzt. Cache auf `TTLCache(maxsize=…)` umstellen. | `routes/content.py:122`, `content_db_service.py:57-80` | *(umgesetzt als Muster `^[a-z][a-z0-9_]{0,31}$`, da die Typen nicht sauber enumerierbar sind; Cache auf 128 Einträge begrenzt)*
| ✅ | `POST /api/stats` bekommt `@limiter.limit`, Highscore-Einträge ohne `user_id` werden aus `/stats/highscores` ausgeschlossen. | `routes/stats.py:25` | *(Rate-Limit 10/h umgesetzt; der Ausschluss anonymer Einträge aus den Highscores ist eine Produktentscheidung und bleibt offen)*
...
| ✅ | Vitest in zwei Projekte teilen: `unit` (PR-Gate, < 60 s) und `balance` (nur `balance-check.yml`). | Die Balance-Sim macht 280 von 288 s der Suite aus und läuft heute doppelt. | *(umgesetzt über `--exclude` in den npm-Skripten und `npm run test:balance`)*
| ✅ | `pytest-cov` mit `--cov-fail-under=70` im `backend-pytest-db`-Job; den DB-losen `backend-tests`-Job streichen. | Backend-Coverage wird nicht gemessen; der DB-lose Job überspringt 27 % still und ist redundant. | *(Schwelle 55 %, Ist 61 % inkl. Branch-Coverage)*

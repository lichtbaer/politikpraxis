...
- **Per-Worker-Limits:** Das Backend startet in Produktion mit `--workers 2` ([`Dockerfile.prod`](https://github.com/lichtbaer/politikpraxis/blob/main/backend/Dockerfile.prod)). In-Memory-Buckets (slowapi/Auth, Kontakt-Limiter) gelten weiterhin **pro Worker**; die effektiven Limits können sich dadurch annähernd verdoppeln.

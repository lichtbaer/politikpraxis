...
def upgrade() -> None:
    ...
    aktionen_i18n_de = [
        (
            "wahlkampf_rede",
            "Wahlkampfrede",
            "Große Rede vor Parteibasis oder Bürgern. Stärkt Zustimmung in der Wählerschaft.",
        ),
        (
            "wahlkampf_tv_duell",
            "TV-Duell",
            "Einmaliges TV-Duell mit Oppositionsführer. Hohe Reichweite, keine PK-Kosten.",
        ),
        (
            "wahlkampf_koalition",
            "Koalitionsauftritt",
            "Gemeinsamer Auftritt mit Koalitionspartner. Stärkt Koalitionsimage.",
        ),
        (
            "wahlkampf_milieu",
            "Milieu-Ansprache",
            "Gezielte Ansprache eines Wählermilieus. Erhöht Zustimmung in dieser Gruppe.",
        ),
        (
            "wahlkampf_medien",
            "Medienoffensive",
            "Einmalige große Medienkampagne. Maximale Sichtbarkeit.",
        ),
        (
            "wahlkampf_verbands",
            "Verbandsarbeit",
            "Passive Pflege von Verbandsbeziehungen. Keine PK-Kosten, wirkt im Hintergrund.",
        ),
    ]
    ...
    aktionen_i18n_en = [
        (
            "wahlkampf_rede",
            "Campaign Speech",
            "Major speech before party base or citizens. Strengthens voter approval.",
        ),
        (
            "wahlkampf_tv_duell",
            "TV Debate",
            "One-time TV debate with opposition leader. High reach, no PK cost.",
        ),
        (
            "wahlkampf_koalition",
            "Coalition Appearance",
            "Joint appearance with coalition partner. Strengthens coalition image.",
        ),
        (
            "wahlkampf_milieu",
            "Milieu Outreach",
            "Targeted outreach to a voter milieu. Increases approval in that group.",
        ),
        (
            "wahlkampf_medien",
            "Media Offensive",
            "One-time major media campaign. Maximum visibility.",
        ),
        (
            "wahlkampf_verbands",
            "Association Work",
            "Passive maintenance of association relations. No PK cost, works in background.",
        ),
    ]
...
def downgrade() -> None:
    ...
    for aid in aktion_ids:
        conn.execute(
            sa.text("DELETE FROM wahlkampf_aktionen_i18n WHERE aktion_id = :id"),
            {"id": aid},
        )
        conn.execute(
            sa.text("DELETE FROM wahlkampf_aktionen WHERE id = :id"), {"id": aid}
        )

...
Das Frontend spricht die API über `frontend/src/services/api.ts` an (`apiFetch`, Basis-URL aus `VITE_API_URL`). JWT-Access-Token aus dem `authStore` werden mitgeschickt, Refresh läuft über ein HttpOnly-Cookie. Content wird beim Start über `GET /api/content/game?locale=` geladen (`contentStore`); schlägt das fehl, greift ein lokaler Offline-Fallback (`frontend/src/data/defaults/`) mit eingeschränktem Content (siehe #244 zur geplanten Reduktion dieses Fallbacks). Die Produktion baut das Frontend und liefert es über nginx aus; API-Anfragen werden per Proxy an den Backend-Container weitergeleitet (`/api` → Backend). Im Dev gilt dasselbe Prinzip: `VITE_API_URL` bleibt auf dem Default `/api`, und der Vite-Dev-Proxy (`frontend/vite.config.ts`, Ziel via `VITE_DEV_API_PROXY_TARGET`) leitet weiter. Frontend und API teilen sich also in allen Umgebungen eine Origin — es gibt keine CORS-Preflights, und das HttpOnly-Refresh-Cookie ist same-origin. `CORS_ORIGINS` deckt nur Sonderfälle ab.
...
Alle `fetch_*`-Funktionen sowie `get_game_content_from_db` in `content_db_service.py` cachen ihr Ergebnis für `CACHE_TTL` (60 Sekunden) in einem Prozess-lokalen Dict (`_content_cache`, Key `(entity, locale)`). Admin-Writes (`admin_*.py`) rufen `content_cache_clear()` auf, um den Cache nach Änderungen sofort zu leeren.

**Bekannter Trade-off (#249):** Der Cache liegt in Prozess-Memory. Bei mehreren Uvicorn-Workern (`docker-compose.prod.yml`, `--workers 2`) hat jeder Worker seinen eigenen Cache — `content_cache_clear()` nach einem Admin-Write leert nur den Cache des Workers, der den Request bearbeitet hat. Die übrigen Worker liefern bis zu `CACHE_TTL` (60s) veralteten Content aus, bis ihr eigener Cache regulär abläuft. Für Admin-Content-Änderungen (seltene, manuelle Aktion) wird dies bewusst in Kauf genommen statt eine geteilte Cache-Infrastruktur (Redis o. ä.) einzuführen — die TTL wurde deshalb bewusst kurz gehalten statt der ursprünglichen 1h, um das worker-übergreifende Staleness-Fenster klein zu halten. Sollte auch das zum Problem werden, ist ein geteilter Cache mit Pub/Sub-Invalidierung der nächste Schritt (siehe auch #231 zu geteiltem Storage für Rate-Limits — eine gemeinsame Redis-Instanz könnte beide Themen lösen).
